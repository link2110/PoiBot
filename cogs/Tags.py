...
"""
This cog is written by Danny, with some modifications from myself. https://github.com/Rapptz
"""

blacklist = ["147143652748034048", "143519913053650944", "178777673911238656", "143229371442921473", "125367104336691200", "144486179205545985", "151626860210552832", "143523696366190592", "139966249248489472", "184212523334107137", "202106762705240064"]
...
class Tags:
    ...
    async def create(self, ctx, name : str, *, content : str):
        ...
        The tag you create can only be accessed in the server that it was created in.
        """
    ...
    async def make(self, ctx):
        ...
        This walks you through the process of creating a tag with8
        ...
        await self.bot.say('What would you like the tag name to be?')
        ...
        if name is None:
            await self.bot.say('{} You took too long. Please start again.'.format(ctx.message.author.mention))
            return
        ...
        await self.bot.say('So the name is {0.content}. What about the tag\'s content?'.format(name))
        ...
        await self.config.put(location, db)
        await self.bot.say('Okay. I\'ve made your {0.content} tag.'.format(name))
    ...
    async def remove(self, ctx, *, name : str):
        ...
        deletion and has Manage Messages permissions or Poi's Admiral role then
        they can also remove tags from the server-specific database.
        """
        lookup = name.lower()
        server = ctx.message.server
        tag = self.get_tag(server, lookup)
        ...
        if not tag.is_generic:
            can_delete = checks.role_or_permissions(ctx, lambda r: r.name == "Poi's Admiral", manage_messages=True)
        else:
    ...
    async def _list(self, ctx, *, member : str = None):
        """
        Lists all the tags that belong to you or someone else.
        """
        if len(ctx.message.mentions) <1:
            owner = ctx.message.author if member is None else await getmember(member, ctx.message)
        else:
        ...
        if tags:
            ...
            await self.bot.say(fmt.format(owner, ', '.join(tags).replace("@", "@\u200b")))
        else:
